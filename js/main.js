...
jQuery('.gallery-option').hide();
	if (jQuery(window).scrollTop() >= jQuery(window).innerHeight()) {
		jQuery('#top').css({ backgroundColor: '#000000'});
	} else {
		jQuery('#top').css({ backgroundColor: '' });
	}
});

jQuery(window).on("scroll", function() {
var limit = jQuery('body').is('.page-id-255') ? 320 : jQuery(window).innerHeight();
...
jQuery("#parallax-sections-69").parallax("50%", "0.3");
jQuery("#parallax-sections-72").parallax("50%", "0.3");
jQuery("#parallax-sections-109").parallax("50%", "0.3");
});
